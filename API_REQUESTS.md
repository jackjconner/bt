...
what it has this round. The head agent routes producers next round. A request is
closed only when the producer adds the field **additively** (new column/field
with a default; no signature break) and the consumer's gate confirms it lands.
Driven by the `component-improvement-loop` skill.
